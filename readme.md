...
在大型服务器中，经过会出现描述符用完（通常是 linux 的资源限制导致的）的情况，这会导致 accept() 失败，并返还一个 ENFILE 错误，但是内核并没有拒绝这个连接，连接仍然在连接队列中，这导致在下一次迭代的时候，仍然会触发监听描述符的可读事件，最终造成程序 busy loop。一种简单的处理方式就是当程序遇到这种问题就直接忽略掉，直到这种情况消失，显然这种方法将会导致 `busy waiting`，一种比较好的处理方式就是记录除了 EAGAIN 或 EWOULDBLOCK 其他任何错误，告诉用户出现了某种错误，并停止监听描述符的可读事件，减少 CPU 的使用。如果程序是单线程的，我们可以先 open /dev/null，保留一个描述符，当 accept() 出现 ENFILE 或 EMFILE 错误的时候，close 掉 /dev/null 这个 fd，然后 accept，再 close 掉 accept 产生的 fd，然后再次 open /dev/null，这是一种比较优雅的方式来拒绝掉客户端的连接。最后一种方式则是遇到 accept() 的这种错误，直接拒绝并退出，但是显然这种方式很容易受到 Dos 攻击。而 Acceptor 对象仅在 Acceptor 线程中受理客户端连接，符合上述单线程优雅处理方式，代码片段如下：
...
learn 目录下是一些测试程序和工具书籍，工具书籍只在需要用到的部分写了测试代码，所以许多章节代码不全。此外测试代码中注释可能比较凌乱

### apue

《Unix环境高级编程》是一部具有开创性质的梳理性书籍，对许多技术的来龙去脉都讲的十分清除，而且翻译人员十分用心，在中文阅读上不会有奇怪感（在线吐槽并感谢 C++ 并发实践的翻译，让我养成了阅读英文原版的习惯），不过该书一方面比较老了，另一方面样例代码比较少，适合作为大纲类的书籍，每一章配合 man 手册来看。

### tcpip

《TCP/IP 网络编程》是我最早看的网络编程相关书籍，该书多数章节的样例代码我都修改并测试了，适合初学，但不能满足深入学习的需求。

### hplsp

《Linux 高性能服务器编程》中我经常反复看的部分有 TCP 的状体迁移图和非阻塞 connect 部分，并且本书在 github 上附带了一个完整的服务器样例 TinyWebServer，样例中包含了日志，定时器，线程池，数据库连接池，http 解析状态机，Reactor 模式事件处理，他也是我第一个完整阅读并学习的项目。

### tlpi

《Linux/Unix 系统编程手册》 绝对是相见恨晚的书，只恨自己最近几天才发现它而没时间阅读，从中英两版的阅读体验上来看，翻译的非常自然，能感觉为了表达准确重新组织语言甚至不完全按照原文翻译的用心。目前读过的章节都写了测试代码，为了学习方便，我还专门写了一个测试用的 header 头文件。
