...
/**
 * 在执行 epoll_ctl 调用前 close(clnt_sock)，会直接导致 EBADF: Bad file descriptor 错误
 */
void Server()
{
    ...
    /// 在 epoll_ctl 调用前关闭 clnt_sock 会在 epoll_ctl 调用时触发如下错误
    /// [EBADF: Bad file descriptor]EBADF: epfd or fd is not a valid file descriptor!
    close(clnt_sock);
