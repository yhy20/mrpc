...
/**
 * 在执行 epoll_wait 调用前 close(clnt_sock)，从 epoll set 中清除 clnt_sock, 不会触发任何事件 
 */
void Server()
{
    ...
    /**
     * 在 epoll_ctl 添加 clnt_sock 后关闭 clnt_sock
     * 本质上该问题是 close 函数对 epollfd 的影响，详见 man 7 epoll Q6 A6
     ...
     * -iption remain open.
     * 
     * 此处 clnt_sock 的引用计数为 1，所以 close 会将 clnt_sock 直接从 epoll set 中移除
     * 后续的 epoll 无法监听到任何事件
     */
    close(clnt_sock); 
    sleep(1);
